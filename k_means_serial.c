...
int *cluster(double **data_points_normalized,int *initial_centroids,int no_of_rows,int no_of_columns,int no_of_clusters){
  ...
  while(re_assignments!=0){
    ...
    for(i=0;i<no_of_rows;i++){
      int allocated_to=12345563;
      int previous_to=labels[i];
      double error_for_this_point=999999999999;
      /* printf("%d\n",previous_to); */
      for(j=0;j<no_of_clusters;j++){
	double fook=(eucledian_distance(data_points_normalized[i],centroids[j],no_of_columns));
	if(fook<error_for_this_point){
      ...
      /* printf("%d %d\n",previous_to,allocated_to); */
      points_in_each_cluster[previous_to]--;
      points_in_each_cluster[allocated_to]++;
      if(previous_to!=allocated_to)
	re_assignments++;
      labels[i]=allocated_to;
      error+=error_for_this_point;
    }
...
int main(int argc, char* argv[]){
  ...
  sprintf(out_file,"labels_%d_%d.csv", no_of_clusters, seed);
